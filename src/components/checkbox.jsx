...
export function Checkbox({type = "checkbox", id = "checkbox", selected = false, disabled = false, hover = false, active = false, children, ...props}) {
    
    const checkboxClasses = "flex gap-4";
    
    const types = {
        checkbox: "m-0 appearance-none w-6 h-6 border-[0.25em] border-solid border-purple-500 cursor-pointer grid place-content-center " +
    ...
    "w-6 h-6 rounded-full border-[0.25em] border-purple-500 bg-white " +
    ...
    return (
        <div className={checkboxClasses}>
            <input 
                ...
                className={twMerge(types[type], isSelected[selected])}
                > 
            </input>
            <label htmlFor={id}>
                {children}
